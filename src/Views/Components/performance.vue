<template>
  <div class="py-4 container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <div class="row">

          <div class="w-100 d-flex flex-gap-20">
            <div >

              <UserKpiComponent 
              name="Yaw Graham"
              roll="Distributor"
              status="true"
              usercard="user-card-27"
              >
                <template #profile>
                  <img src="../../assets/img/team-1.jpg " alt="" class="user-profile"  style="width:73px"/>
                </template>
              </UserKpiComponent>
            </div>
            <div >

              <UserKpiComponent 
              usercard="user-card-35"
              userdetails="true"
              />
            </div>
          </div>


          <KpiComponents />

          <div class="mt-4 row">
            <div class="mt-4 col-md-6 mt-md-0">
              <gradient-line-chart id="chart-line" title="Overall Sales" description="<i class='fa fa-arrow-up text-success'></i>
      <span class='font-weight-bold'>4% more</span> in 2021" :chart="{
        labels: [
          'Apr',
          'May',
          'Jun',
          'Jul',
          'Aug',
          'Sep',
          'Oct',
          'Nov',
          'Dec',
        ],
        datasets: [
          {
            label: 'Mobile Apps',
            data: [50, 40, 300, 220, 500, 250, 400, 230, 500],
          },
        ],
      }" />
            </div>

            <div class="col-md-6">
              <bar-chart title="Overall Sales" height="300" :chart="{
                labels: ['16-20', '21-25', '26-30', '31-36', '36-42', '42+'],
                datasets: {
                  label: 'Sales by age',
                  data: [15, 20, 12, 60, 20, 15],
                },
              }" />
            </div>

            <div class="mt-4 row ">
              <div class="col-lg-3 col-md-6 col-12 mt-3 ">
                <line-chart id="chart-widgets-1" title="Sales" title2="3kg | Steel" :value="{
                  amount: 'GHs 90',
                  percentage: { value: '+5% since last month', color: 'success' },
                }" :chart="{
  labels: [
    'Apr',
    'May',
    'Jun',
    'Jul',
    'Aug',
    'Sep',
    'Oct',
    'Nov',
    'Dec',
  ],
  datasets: {
    label: 'Income',
    data: [50, 45, 60, 60, 80, 65, 90, 80, 100],
  },
}" />
              </div>
              <div class="col-lg-3 col-md-6 col-12 mt-3 col-4">
                <line-chart id="chart-widgets-2" title="Sales" title2="3kg | Steel" :value="{
                  amount: 'GHs 93',
                  percentage: { value: '+6% since last month', color: 'success' },
                }" :chart="{
  labels: [
    'Apr',
    'May',
    'Jun',
    'Jul',
    'Aug',
    'Sep',
    'Oct',
    'Nov',
    'Dec',
  ],
  datasets: {
    label: 'Income',
    data: [50, 45, 60, 60, 80, 65, 90, 80, 100],
  },
}" />
              </div>
              <div class="col-lg-3 col-md-6 col-12 mt-3 col-4">
                <line-chart id="chart-widgets-3" title="Sales" title2="3kg | Steel" :value="{
                  amount: 'GHs 93',
                  percentage: { value: '+6% since last month', color: 'success' },
                }" :chart="{
  labels: [
    'Apr',
    'May',
    'Jun',
    'Jul',
    'Aug',
    'Sep',
    'Oct',
    'Nov',
    'Dec',
  ],
  datasets: {
    label: 'Income',
    data: [50, 45, 60, 60, 80, 65, 90, 80, 100],
  },
}" />
              </div>
              <div class="col-lg-3 col-md-6 col-12 mt-3 col-4">
                <line-chart id="chart-widgets-4" title="Sales" title2="3kg | Steel" :value="{
                  amount: 'GHs 93',
                  percentage: { value: '+6% since last month', color: 'success' },
                }" :chart="{
  labels: [
    'Apr',
    'May',
    'Jun',
    'Jul',
    'Aug',
    'Sep',
    'Oct',
    'Nov',
    'Dec',
  ],
  datasets: {
    label: 'Income',
    data: [50, 45, 60, 60, 80, 65, 90, 80, 100],
  },
}" />
              </div>
              <div class="col-lg-3 col-md-6 col-12 mt-3 col-4">
                <line-chart id="chart-widgets-5" title="Sales" title2="3kg | Steel" :value="{
                  amount: 'GHs 93',
                  percentage: { value: '+6% since last month', color: 'success' },
                }" :chart="{
  labels: [
    'Apr',
    'May',
    'Jun',
    'Jul',
    'Aug',
    'Sep',
    'Oct',
    'Nov',
    'Dec',
  ],
  datasets: {
    label: 'Income',
    data: [50, 45, 60, 60, 80, 65, 90, 80, 100],
  },
}" />
              </div>
            </div>
            <div class="row">
              <OrderList />
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import KpiComponents from "@/components/cards/KpiComponents.vue";
import UserKpiComponent from "@/components/cards/UserKpiComponent.vue";
import BarChart from "@/components/Charts/BarChart.vue";
import GradientLineChart from "@/components/Charts/GradientLineChart.vue";
import LineChart from "@/components/Charts/LineChart.vue";
import OrderList from "@/components/OrderList.vue";
export default {
  name: "PerformanceCompo",
  components: {
    BarChart,
    GradientLineChart,
    LineChart,
    OrderList,
    KpiComponents,
    UserKpiComponent
  },
};
</script>

<style lang="scss" scoped></style>
